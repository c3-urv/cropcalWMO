...
ui <- fluidPage(
  ...
  tags$head(tags$style(
    HTML(".shiny-notification {position:fixed;top: 30%;left: 30%;right: 30%;}"))),
...
#           numericInput("mestart", "Start Month:", value = 1, width = "60%"),
           ...
           actionButton("generatebatch", "Run Batch Mode", width = "80%"),
...
#        mestart = input$mestart,
  ...
  observeEvent(input$generatebatch, {
    outputText <- isolate({
      infolder <- input$infolder
      iter <- read_table(paste0(infolder,'/batch.txt'))
      pn <- nrow(iter)
      
      # Inicializar la barra de progreso
      withProgress(message = 'Saving to Output Folder ...', 
                   value = 0,
                   
                   {
        for(i in 1:pn) {
          # Actualizar el valor de la barra de progreso
          incProgress(1/pn, detail = paste("Station", iter$estacion[i] ,i, "of", pn))
