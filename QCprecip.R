...
if(!require(INQC)){install.packages('INQC')};library(INQC)
if(!require(DT)){install.packages('DT')};library(DT)
if(!require(crosstalk)){install.packages('crosstalk')};library(crosstalk)
...
extract_station_data <- function(prr,ptx,ptn, output_dir = "../qcdata", input_dir ='../data') {
  ...
  for (station in nyustations) {
    ...
    RR <- prr %>% filter(Station == station) %>% dplyr::select(Year,Month,Day,rr) 
    RX <- ptx %>% filter(Station == paste0('TX', substring(station,3,nchar(station)))) %>% mutate(tx = rr) %>% dplyr::select(Year,Month,Day,tx) 
    RN <- ptn %>% filter(Station == paste0('TN', substring(station,3,nchar(station)))) %>% mutate(tn = rr) %>% dplyr::select(Year,Month,Day,tn) 
...
rr <- NULL
QClist <- NULL
nyu_default <- 500  # Default value for max rainfall allowd
seco_default <- 200 # Default value for max number of consecutive dry days
...
rutaestaciones<-NULL 
sustu_default<-100 # Default sustpectacumprec, value
sustitu_default<-10 # Default suspectacumprec, number of previous days
jedi_default <- 100 # Default return period for pareto distro (jedi returns jojoo)
zero_default<-7 # Default IQR, number of IQRs
...
# UI
ui <- fluidPage(
  titlePanel("QC Rainfall Data with INQC"),
  fluidRow(
    column(3, 
           textInput("directory", "Path to data folder (either relative or full)",value = '../data'),
           actionButton("showFiles", "Load Data"),
           actionButton("runINQC", "Run INQC for Precipitation"),
           actionButton("extractStationData", "Extract Station Data"),
           textInput("outputDirectory", "Output Directory:", value = "../qcdataRR"),
           actionButton("cerrar", "Exit App.", icon = icon("times")),

    ),
    
    column(1,
           numericInput("nyuInput", "Max Rainfall:", value = nyu_default),
           ...
           numericInput("margenInput","Quantile Diffs", value = margen_default),
           
    ),
    column(1,
           numericInput("secoInput","Max CDDs", value = seco_default),
           numericInput("humedoInput","Max CWDs", value = humedo_default),
           numericInput("repetidoInput","Max Dec. Month ", value = repetido_default),
           numericInput("vecesInput","Diffs multiplier", value = veces_default),
           
           
    ),
    
    column(1,
           numericInput("sustuInput","Suspect Accum.", value = sustu_default),
           numericInput("sustituInput","Previous Days", value = sustitu_default),
           numericInput("jediInput","Return Period", value = jedi_default),
    ),
    column(1,
...
server <- function(input, output, session) {
  ...
  observeEvent(input$showFiles, {
    ...
    if (dir.exists(data_dir)) {
      ...
      # Pivot para TX
      rrTX <<- data_list %>% dplyr::select(-RR, -TN) %>%
  ...
  observeEvent(input$runINQC, {
    ...
    ## SMALL
    small <- rr[, 5:ncol(rr)] %>% lapply(function(x) INQC::physics(x, nyu = 0, compare = 3)) %>% keep(~length(.x) > 0)
    ...
    dryseq <- rr[, 5:ncol(rr)] %>% lapply(function(x) drywetseq(x, chseco=secopar, sueco = 0, dry=TRUE, wet = FALSE)) %>% keep(~length(.x) > 0) %>%
      lapply(function(x) c(x[which(diff(x)!=1)], tail(x, 1)))
    QClist<-bind_rows(QClist,rr%>% mutate_at(vars(5:ncol(rr)),~ ifelse(is.na(.), 9, 0)) %>% updater(.,dryseq,4,TRUE) %>%
                        filter(if_any(5:last_col(), ~.x != 0 & .x != 9)) %>% mutate(TEST = 'dryseq'))
    ## WETSEQ
    
    wetseq <- rr[, 5:ncol(rr)] %>% lapply(function(x) drywetseq(x, chhumedo = humedopar, sueco = 0, dry=FALSE, wet = TRUE)) %>% keep(~length(.x) > 0) %>%
      lapply(function(x) c(x[which(diff(x)!=1)], tail(x, 1)))
    QClist<-bind_rows(QClist,rr%>% mutate_at(vars(5:ncol(rr)),~ ifelse(is.na(.), 9, 0)) %>% updater(.,wetseq,4,TRUE) %>%
                        filter(if_any(5:last_col(), ~.x != 0 & .x != 9)) %>% mutate(TEST = 'wetseq'))
    # 
    # ## FLAT
    # 
    ...
    # 
     ## REPEATEDVALUE
    # 
     repeatedvalue<-rr[, 5:ncol(rr)] %>% lapply(function(x) INQC::repeatedvalue(x, margin=10,friki = quantile(x[x!=0],0.90, na.rm=TRUE))) %>% keep(~length(.x) > 0)
     QClist<-bind_rows(QClist,rr%>% mutate_at(vars(5:ncol(rr)),~ ifelse(is.na(.), 9, 0)) %>% updater(.,repeatedvalue,4,TRUE) %>%
                         filter(if_any(5:last_col(), ~.x != 0 & .x != 9)) %>% mutate(TEST = 'repeatedvalue'))
    # 
    # ## PARETO
    # 
     pareto<-rr[, 5:ncol(rr)] %>% lapply(function(x) INQC::paretogadget(x, ret=jedi)) %>% keep(~length(.x) > 0)
     QClist<-bind_rows(QClist,rr%>% mutate_at(vars(5:ncol(rr)),~ ifelse(is.na(.), 9, 0)) %>% updater(.,pareto,3,TRUE) %>%
                         filter(if_any(5:last_col(), ~.x != 0 & .x != 9)) %>% mutate(TEST = 'pareto'))
    # 
    # 
    # ## NEWFRIKI
    ...
    # ## SUSPECTACUM
    # 
     susacum <- lapply(rr[, 5:ncol(rr)], function(x) {
       x_numeric <- as.numeric(x)
       suspectacumprec(data.frame(Date = rr$Date, Value = x_numeric), sustu,sustitu)
     }) %>% keep(~length(.x) > 0)  %>% lapply(function(x) c(x[which(diff(x)!=1)], tail(x, 1)))
     QClist<-bind_rows(QClist,rr%>% mutate_at(vars(5:ncol(rr)),~ ifelse(is.na(.), 9, 0)) %>% updater(.,susacum,2,TRUE) %>%
                         filter(if_any(5:last_col(), ~.x != 0 & .x != 9)) %>% mutate(TEST = 'susacum'))
    # 
    # 
    # 
    # ## ROUNDPREC
    roundprec <- lapply(rr[, 5:ncol(rr)], function(x) { ### issue: considers .5 and 5 as a hit! Might be solvable round(x,1)
      x_numeric <- as.numeric(x) %>% keep(~length(.x) > 0)
      roundprecip(data.frame(Date = rr$Date, Value = x_numeric), blocksize=repetido, exclude=0)
    }) %>% keep(~length(.x) > 0)
    QClist<-bind_rows(QClist,rr%>% mutate_at(vars(5:ncol(rr)),~ ifelse(is.na(.), 9, 0)) %>% updater(.,roundprec,4,TRUE) %>%
                        filter(if_any(5:last_col(), ~.x != 0 & .x != 9)) %>% mutate(TEST = 'roundprec'))
    filtro <- QClist %>% filter(TEST == "roundprec") %>% group_by(Year, Month) %>% arrange(Day) %>% filter(Day == max(Day)) %>% arrange(Year, Month)
    QClist<-QClist %>% filter(TEST != 'roundprec') %>% bind_rows(filtro)
    ...
    rrpivot_data <-rr %>% pivot_longer(starts_with('RR')) %>% rename(rr = value, Station = name) %>%
       arrange(Station,Year,Month,Day) %>% mutate(row_id = row_number(), Date = ymd(Date))
    
    rrpivotTX_data <-rrTX %>% pivot_longer(starts_with('TX')) %>% rename(rr = value, Station = name) %>%
      arrange(Station,Year,Month,Day) %>% mutate(row_id = row_number(), Date = ymd(Date))
    ...
    QCrr_data<- QClist %>% pivot_longer(cols=starts_with('RR')) %>% filter(value !=9, value != 0) %>% rename(QC = value, Station = name) %>%
    ...
    rrv$RRpivot <- rrpivot_data
    rrv$TXpivot <- rrpivotTX_data
  ...
  output$selectedRecordPlot <- renderPlot({
    ...
    if (length(selected_rows) > 0) {
      ...
      selected_boxplot <- rrv$RRpivot %>%
        filter(Station == as.character(station), Month == as.numeric(month), rr > 0)
      
      selected_barplot <- rrv$RRpivot %>%
      ...
      selected_triplot<- rrv$RRpivot %>% filter(Station == as.character(station) )%>% filter(Month == as.numeric(month)) %>%
        group_by(Year) %>% summarise(Normal = sum(rr,na.rm=TRUE))
...
plot3<- ggplot(selected_triplot, aes(x = as.factor(Year), y = Normal)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Sum of 'rr' for Each Year",
       x = "Year",
       y = "Total 'rr'") +
  scale_x_discrete(breaks = selected_triplot$Year[selected_triplot$Year %% 5 == 0]) +
  scale_fill_manual(values = ifelse(selected_triplot$Year == as.factor(nyear), "red", "skyblue")) +
  theme_minimal()
  ...
  output$selectedRecordTable <- renderRHandsontable({
    ...
    # Crear un rango de fechas
      if(tiesto == 'wetseq' | tiesto == 'dryseq' | tiesto == 'susacum' | tiesto == 'flat' | tiesto == 'flatfloat' 
       ...
       if(tiesto== 'dryseq'){firsty<- rrv$RRpivot %>% filter(row_id <= idito) %>% slice(which(rr!=0)) %>% last() %>% pull(row_id)
       lasty<-rrv$RRpivot %>% filter(row_id == idito) %>% pull(row_id)}
       if(tiesto== 'wetseq'){firsty<- rrv$RRpivot %>% filter(row_id <= idito) %>% slice(which(rr==0)) %>% last() %>% pull(row_id)
       lasty<-rrv$RRpivot %>% filter(row_id == idito) %>% pull(row_id)}
       if(tiesto == 'susacum' | tiesto == 'flat' | tiesto == 'flatfloat'){firsty = idito -14; lasty = idito+5}  
       if(tiesto == 'roundprec' | tiesto == 'toomonth'){
        
         mes<-rrv$RRpivot %>% filter(Year == nyear, Month == month,Station == station)  %>% pull(row_id)
         firsty=min(mes);lasty=max(mes)
         
       }  
        if(tiesto == 'tooyear'){
          anyo<-rrv$RRpivot %>% filter(Year == nyear,Station == station)  %>% pull(row_id)
    ...
    # Filtrar los registros circundantes
      ...
      surrounding_records <- rrv$RRpivot %>% filter(Station == as.character(station), row_id >= firsty, row_id <= lasty) %>%
    ...
    modalTableData$data <- surrounding_records
      ...
      surrounding_records <- rrv$RRpivot %>%
        filter(Station == as.character(station), Year == as.numeric(nyear), Month == as.numeric(month)) %>% dplyr::select(-Date, -Station)
      modalTableData$data <- surrounding_records
    }
    # Renderizar la tabla
  ...
  observeEvent(input$saveButton, {
    ...
    # Obtener los datos actualizados de la tabla modal
    selectedData <- hot_to_r(input$selectedRecordTable)
    
    # Paso 1: Identificar qué filas de rrv$QCrr hay que actualizar
    rows_to_update <- which(rrv$QCrr$row_id %in% selectedData$row_id)
    
    # Paso 2: Filtrar rrv$QCrr y selectedData para las filas comunes
    rrv_QCrr_filtered <- rrv$QCrr[rows_to_update, ]
    selectedData_filtered <- selectedData[selectedData$row_id %in% rrv$QCrr$row_id, ]
    
    # Paso 3: Actualizar rrv$QCrr con los valores correspondientes de selectedData
    if (nrow(rrv_QCrr_filtered) > 0 && nrow(selectedData_filtered) > 0) {
      columns_to_update <- intersect(names(rrv_QCrr_filtered), names(selectedData_filtered))
      rrv$QCrr[rows_to_update, columns_to_update] <- selectedData_filtered[, columns_to_update]
    } else {
      print("No rows to update or no common rows.")
    }
    
    # Crear una versión similar para rrv$RRpivot
    rows_to_update_pivot <- which(rrv$RRpivot$row_id %in% selectedData$row_id)
    rrv_rrpivot_filtered <- rrv$RRpivot[rows_to_update_pivot, ]
    selectedData_filtered_pivot <- selectedData[selectedData$row_id %in% rrv$RRpivot$row_id, ]
    
    if (nrow(rrv_rrpivot_filtered) > 0 && nrow(selectedData_filtered_pivot) > 0) {
      columns_to_update_pivot <- intersect(names(rrv_rrpivot_filtered), names(selectedData_filtered_pivot))
      rrv$RRpivot[rows_to_update_pivot, columns_to_update_pivot] <- selectedData_filtered_pivot[, columns_to_update_pivot]
    
      ### elimino desde aquí la fila
      rrv$QCrr <- rrv$QCrr[-rows_to_update,]
  ...
  observeEvent(input$deleteButton, {
    ...
    # Eliminar las filas de rrv$QCrr y rrv$RRpivot
  ...
  observeEvent(input$misser2, {
    ...
    rrv$QCrr <- rrv$QCrr %>% filter(!row_id %in% target)
    rrv$RRpivot <- rrv$RRpivot %>% mutate(rr = ifelse(row_id %in% target, NA, rr))
