...
rr <- NULL
QClist <- NULL
nyu_default <- 40  # Default value for max tx allowed
seco_default <- -20 # Default value for smallest max temp
...
# UI
ui <- fluidPage(
  titlePanel("QC Tmin Data with INQC"),
  fluidRow(
    column(3, 
           textInput("directory", "Path to data folder (either relative or full)",value = '../qcdataTX'),
           actionButton("showFiles", "Load Data"),
           actionButton("runINQC", "Run INQC for Tmin"),
           actionButton("extractStationData", "Extract Station Data"),
           textInput("outputDirectory", "Output Directory:", value = "../qcdataTN"),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$showFiles, {
    ...
    if (dir.exists(data_dir)) {
      ...
      rr <<- data_list %>% dplyr::select(-RR, -TX) %>%
...
#
    ...
    txtn1<-lapply(5:ncol(rr), function(i) which(rrTX[[i]] <= rr[[i]]));names(txtn1)<-colnames(rr)[5:ncol(rr)]
    ...
    rrpivot_data <-rr %>% pivot_longer(starts_with('TN')) %>% rename(rr = value, Station = name) %>%
    ...
    QCrr_data<- QClist %>% pivot_longer(cols=starts_with('TN')) %>% filter(value !=9, value != 0) %>% rename(QC = value, Station = name) %>%
  ...
  output$selectedRecordTable <- renderRHandsontable({
    ...
    # Crear un rango de fechas
      if(tiesto == 'wetseq' | tiesto == 'dryseq' | tiesto == 'susacum' | tiesto == 'flat' | tiesto == 'flatfloat' 
       ...
       if(tiesto== 'dryseq'){firsty<- rrv$TNpivot %>% filter(row_id <= idito) %>% slice(which(rr!=0)) %>% last() %>% pull(row_id)
       lasty<-rrv$TNpivot %>% filter(row_id == idito) %>% pull(row_id)}
       if(tiesto== 'wetseq'){firsty<- rrv$TNpivot %>% filter(row_id <= idito) %>% slice(which(rr==0)) %>% last() %>% pull(row_id)
       lasty<-rrv$TNpivot %>% filter(row_id == idito) %>% pull(row_id)}
       if(tiesto == 'susacum' | tiesto == 'flat' | tiesto == 'flatfloat'){firsty = idito -14; lasty = idito+5}  
       if(tiesto == 'roundprec' | tiesto == 'toomonth'){
        
         mes<-rrv$TNpivot %>% filter(Year == nyear, Month == month,Station == station)  %>% pull(row_id)
         firsty=min(mes);lasty=max(mes)
         
       }  
        if(tiesto == 'tooyear'){
          anyo<-rrv$TNpivot %>% filter(Year == nyear,Station == station)  %>% pull(row_id)
    ...
    # Filtrar los registros circundantes
      ...
      surrounding_records <- rrv$TNpivot %>% filter(Station == as.character(station), row_id >= firsty, row_id <= lasty) %>%
      left_join(rrv$TXpivot,by='row_id',suffix = c('','.y')) %>% rename(rrTX = rr.y) %>% 
        dplyr::select(-ends_with('.y')) %>%
      select(-Station,  -Date) %>% select(Year, Month, Day, rrTX,rr, row_id)
    ...
    modalTableData$data <- surrounding_records
      ...
      surrounding_records <- rrv$TNpivot %>%
        filter(Station == as.character(station), Year == as.numeric(nyear), Month == as.numeric(month)) %>% 
        left_join(rrv$TXpivot,by='row_id',suffix = c('','.y')) %>% rename(rrTX = rr.y) %>% 
        dplyr::select(-ends_with('.y')) %>%
        select(-Date, -Station) %>% select(Year, Month, Day, rrTX, rr, row_id)
  ...
  observeEvent(input$saveButton, {
    ...
    sustis<-inner_join(selectedData,rrv$QCrr,by='row_id',suffix =c('','.y'))
    ...
    sustis<-inner_join(selectedData,rrv$TXpivot,by='row_id',suffix =c('','.y')) %>% select(-ends_with('.y')) %>% 
      select(Date,Year,Month,Day,Station,rrTX,row_id) %>% rename(rr = rrTX)
  ...
  observeEvent(input$misser2, {
    ...
    rrv$QCrr <- rrv$QCrr %>% filter(!row_id %in% target)
    rrv$TNpivot <- rrv$TNpivot %>% mutate(rr = ifelse(row_id %in% target, NA, rr))
